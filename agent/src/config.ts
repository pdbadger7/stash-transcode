...
const configSchema = z.object({
  ...
  remuxHlsReadRate: z.coerce.number().nonnegative().default(0),
  remuxHlsStartOffsetSeconds: z.coerce.number().nonnegative().default(0.15),
...
export function loadConfig(): Config {
  ...
  const config = configSchema.parse({
    ...
    remuxHlsReadRate: env.REMUX_HLS_READ_RATE,
    remuxHlsStartOffsetSeconds: env.REMUX_HLS_START_OFFSET_SECONDS,
